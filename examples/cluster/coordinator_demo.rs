...
use aidb::cluster::{Coordinator, HealthChecker, HealthCheckConfig, PrimaryNode};
use aidb::{Options, DB};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50051".to_string()).await?;
    coordinator.register_shard("shard2".to_string(), "http://127.0.0.1:50052".to_string()).await?;
    coordinator.register_shard("shard3".to_string(), "http://127.0.0.1:50053".to_string()).await?;
    ...
    for shard_info in shards {
        println!("   {} ({}): {} requests, healthy: {}", 
                 shard_info.id, 
                 shard_info.address,
                 shard_info.request_count,
                 shard_info.healthy);
    }
