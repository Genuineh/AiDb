...
use aidb::cluster::rpc::proto::{
    storage_client::StorageClient, DeleteRequest, GetRequest, PutRequest, ScanRequest,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = tonic::Request::new(GetRequest { key: b"example_key".to_vec() });
    ...
    let request =
        tonic::Request::new(ScanRequest { start_key: vec![], end_key: vec![], limit: 10 });
    ...
    // DELETE operation
    println!("\n=== DELETE Operation ===");
    let request = tonic::Request::new(DeleteRequest { key: b"example_key".to_vec() });
