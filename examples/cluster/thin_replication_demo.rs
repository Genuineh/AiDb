...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let node1 = OpenRaftNode::new(config1, db1, network_factory1).await?;
    ...
    let node2 = OpenRaftNode::new(config2, db2, network_factory2).await?;
    ...
    let node3 = OpenRaftNode::new(config3, db3, network_factory3).await?;
