...
#[cfg(feature = "cluster")]
use aidb::cluster::{
    Coordinator, ConsistentHashRing, HealthChecker, HealthCheckConfig, PrimaryNode,
};
use aidb::{Options, DB};
use std::sync::Arc;
...
fn test_consistent_hash_node_removal() {
    ...
    for i in 0..1000 {
        ...
        if original_mappings[i] != new_mapping {
            changed += 1;
        }
    }
    ...
    assert!(change_rate >= 0.2 && change_rate <= 0.5);
}
...
fn test_consistent_hash_load_distribution() {
    ...
    for (shard_id, count) in distribution {
        let variance = (count as i32 - expected as i32).abs() as f64 / expected as f64;
...
async fn test_coordinator_shard_registration() {
    ...
    let result1 = coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50061".to_string()).await;
    assert!(result1.is_ok());
    
    let result2 = coordinator.register_shard("shard2".to_string(), "http://127.0.0.1:50062".to_string()).await;
...
async fn test_coordinator_routing() {
    ...
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50063".to_string()).await.unwrap();
    coordinator.register_shard("shard2".to_string(), "http://127.0.0.1:50064".to_string()).await.unwrap();
...
async fn test_coordinator_put_and_get() {
    ...
    // Create coordinator and register shards
    let coordinator = Arc::new(Coordinator::new(150));
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50065".to_string()).await.unwrap();
    coordinator.register_shard("shard2".to_string(), "http://127.0.0.1:50066".to_string()).await.unwrap();
...
async fn test_coordinator_delete() {
    ...
    let coordinator = Arc::new(Coordinator::new(150));
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50067".to_string()).await.unwrap();
...
async fn test_coordinator_load_balancing() {
    ...
    // Create coordinator and register shards
    let coordinator = Arc::new(Coordinator::new(150));
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50068".to_string()).await.unwrap();
    coordinator.register_shard("shard2".to_string(), "http://127.0.0.1:50069".to_string()).await.unwrap();
    coordinator.register_shard("shard3".to_string(), "http://127.0.0.1:50070".to_string()).await.unwrap();
...
async fn test_coordinator_shard_unregistration() {
    ...
    // Register shard
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50073".to_string()).await.unwrap();
...
async fn test_health_checker_mark_unhealthy() {
    ...
    // Create coordinator and register shard
    let coordinator = Arc::new(Coordinator::new(150));
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50071".to_string()).await.unwrap();
...
async fn test_health_checker_with_healthy_shard() {
    ...
    // Create coordinator and register shard
    let coordinator = Arc::new(Coordinator::new(150));
    coordinator.register_shard("shard1".to_string(), "http://127.0.0.1:50072".to_string()).await.unwrap();
