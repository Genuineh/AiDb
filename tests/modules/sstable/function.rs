...
use aidb::engine::sstable::{
  parse_sstable_filename, sstable_path, Block, BlockBuilder, Footer, IndexBlock, IndexBlockBuilder,
  IndexEntry, SSTableBuilder, SSTableIterator, SSTableReader, MAGIC_NUMBER,
};
...
fn test_crash_atomicity() {
  ...
  assert!(!path.exists());
  assert!(
    SSTableReader::open(&tmp, None).is_err() || tmp.metadata().unwrap().len() < FOOTER_SIZE as u64
  );
}
