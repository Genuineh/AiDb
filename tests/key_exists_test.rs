...
fn test_key_exists_after_delete_with_flush() {
    ...
    let result = db.get(b"testkey").unwrap();
    assert!(result.is_none(), "Key should NOT exist after delete (before flush), but got: {:?}", result);
    ...
    let result = db.get(b"testkey").unwrap();
    assert!(result.is_none(), "Key should NOT exist after delete and flush, but got: {:?}", result);
}
...
fn test_key_exists_with_empty_value() {
    ...
    assert!(result.is_none(), "Empty value should be treated as tombstone, but got: {:?}", result);
}
...
fn test_reopen_db_after_delete() {
    ...
    // Reopen database
    {
        ...
        assert!(result.is_none(), "Deleted key should not exist after reopen, but got: {:?}", result);
    }
}
