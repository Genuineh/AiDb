...
- **SSTable 压缩 block CRC 损坏 (P0, 默认配置即触发)**: `SSTableBuilder::flush_data_block` 之前用压缩**前** `block_data.len()` 计算 `BlockHandle.size` 并累加 `data_block_offset`, 但 `write_block` 落盘的是压缩**后**的 payload; 启用 `compression` feature 时 (`Options::default()`/`for_high_write_throughput()` 默认就是 `CompressionType::Snap`) 只要一张 SST 里出现第 2 个 data block, 读侧就会用错误的偏移拆 trailer, 读到垃圾 `compression_type`/`crc` 字段, 报 `Corruption("block CRC mismatch")` 甚至直接读故障. 现在 `write_block` 返回实际写入的 payload 长度, 调用方用该返回值计算 `handle.size`. 同时修复了同一批代码里被这个 bug 掩盖的另一个问题: LZ4 压缩用 `prepend_size=false` 写入却用 `decompress(data, None)` 解压 (要求前缀带长度), 二者不匹配导致 LZ4 解压必然失败, 现统一为 `prepend_size=true` 配 `None`. 回归 (此前所有 SSTable 测试都只覆盖 `CompressionType::None`, 掩盖了这两个 bug): `sstable/function.rs::test_multi_block_read_with_snap_compression` / `test_multi_block_read_with_lz4_compression`.
