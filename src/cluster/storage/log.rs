...
impl OpenRaftStorage {
  pub(crate) fn save_vote_internal(&self, vote: &Vote<NodeId>) -> Result<()> {
    let data = rmp_serde::to_vec(vote).map_err(|e| ClusterError::Serialization(e.to_string()))?;
  ...
  pub(crate) fn get_log_entries(
    ...
    for item in iter {
      ...
      let entry: Entry<TypeConfig> =
        rmp_serde::from_slice(&data).map_err(|e| ClusterError::Serialization(e.to_string()))?;
      entries.push(entry);
    }
  ...
  pub(crate) fn load_state(&self) -> Result<()> {
    ...
    if let Some(data) = self.db.get(&snapshot_meta_key(gid))? {
      state.snapshot_meta =
        Some(bincode::deserialize(&data).map_err(|e| ClusterError::Serialization(e.to_string()))?);
    }
  ...
  pub(crate) fn persist_last_applied(&self, log_id: &LogId<NodeId>) -> Result<()> {
    let data = rmp_serde::to_vec(log_id).map_err(|e| ClusterError::Serialization(e.to_string()))?;
