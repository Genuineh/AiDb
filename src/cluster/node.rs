...
impl OpenRaftNode {
  ...
  async fn confirm_replication(&self, voter_ids: &[NodeId]) -> Result<()> {
    ...
    loop {
      let metrics = self.metrics().await;
      let confirmed_voter = voter_ids
        .iter()
        .find(|id| **id != self.node_id() && Self::matched_log_index(&metrics, **id) >= last_log);
  ...
  fn matched_log_index(metrics: &RaftMetrics<NodeId, openraft::BasicNode>, node_id: NodeId) -> u64 {
    metrics
      .replication
      .as_ref()
