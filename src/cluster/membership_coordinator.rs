...
impl MembershipCoordinator {
  ...
  pub async fn add_node(&self, ctx: NodeJoinContext) -> Result<()> {
    ...
    self
      ...
      .await?;
    tracing::info!(elapsed_ms = t0.elapsed().as_millis(), "add_node: RegisterNode committed");
    ...
    self
      ...
      .await?;
    tracing::info!(elapsed_ms = t1.elapsed().as_millis(), "add_node: add_learner committed");
    ...
    tracing::info!(elapsed_ms = t2.elapsed().as_millis(), "add_node: promote_learner_to_voter done");
    ...
    tracing::info!(total_elapsed_ms = start.elapsed().as_millis(), "add_node: complete");
    Ok(())
  }
  ...
  pub async fn remove_node(&self, ctx: NodeLeaveContext) -> Result<()> {
    ...
    if let Some(ref state) = self.meta_raft.get_migration_state() {
      if node_in_active_migration(ctx.node_id, state, &cluster_meta.groups) {
        return Err(ClusterError::InvalidState("node involved in active migration".into()).into());
      }
    }
    ...
    if ctx.force {
      for &gid in &groups_on_node {
        ...
        let new_members: Vec<NodeId> = current_members
          ...
          .collect();
        self
          .change_group_membership(gid, vec![], vec![], Some(new_members))
          .await?;
      }
    } else {
      let meta = self.meta_raft.get_cluster_meta();
      ...
      if !remaining.is_empty() {
        return Err(
          ClusterError::InvalidState(format!(
  ...
  pub async fn replace_node(&self, old_node_id: NodeId, new_node_id: NodeId) -> Result<()> {
    ...
    for gid in groups {
      self
        .change_group_membership(gid, vec![new_node_id], vec![old_node_id], None)
        .await?;
    }
    self
      .meta_raft
