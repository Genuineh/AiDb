...
impl ConsistentHashRing {
    ...
    pub fn new(virtual_nodes: usize) -> Self {
        Self {
            ring: BTreeMap::new(),
            virtual_nodes: if virtual_nodes == 0 { 150 } else { virtual_nodes },
        }
    }
    ...
    pub fn get_node(&self, key: &[u8]) -> Option<ShardId> {
        ...
        for (_node_hash, shard_id) in self.ring.range(hash..) {
...
mod tests {
    ...
    fn test_minimal_redistribution() {
        ...
        assert!(change_rate >= 0.2 && change_rate <= 0.5, 
                "Change rate {} is outside expected range", change_rate);
    }
