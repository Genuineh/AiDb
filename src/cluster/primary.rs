...
use crate::cluster::rpc::{
    self, proto, storage_server::{Storage, StorageServer},
};
...
impl PrimaryNode {
    ...
    pub fn new(db: Arc<DB>) -> Self {
        Self {
            db,
            stats: Arc::new(RwLock::new(PrimaryStats::default())),
        }
    }
...
impl Storage for PrimaryNode {
    ...
    async fn put(&self, request: Request<proto::PutRequest>) -> Result<Response<proto::PutResponse>, Status> {
        let mut stats = self.stats.write();
        stats.total_requests += 1;
    ...
    async fn delete(&self, request: Request<proto::DeleteRequest>) -> Result<Response<proto::DeleteResponse>, Status> {
        let mut stats = self.stats.write();
        stats.total_requests += 1;
    ...
    async fn batch_get(&self, request: Request<proto::BatchGetRequest>) -> Result<Response<proto::BatchGetResponse>, Status> {
    ...
    async fn write(&self, request: Request<proto::WriteRequest>) -> Result<Response<proto::WriteResponse>, Status> {
        self.stats.write().total_requests += 1;
    ...
    async fn health_check(&self, _request: Request<proto::HealthCheckRequest>) -> Result<Response<proto::HealthCheckResponse>, Status> {
        Ok(Response::new(proto::HealthCheckResponse {
            status: proto::health_check_response::ServingStatus::Serving as i32,
        }))
    }

    async fn get_stats(&self, _request: Request<proto::GetStatsRequest>) -> Result<Response<proto::GetStatsResponse>, Status> {
        self.stats.write().total_requests += 1;
