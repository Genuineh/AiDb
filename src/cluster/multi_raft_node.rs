      ...
      Ok((node, storage)) => {
        ...
        if init_as_voter {
          ...
          match node.initialize(vec![(node_id, addr)]).await {
            Ok(()) => {
              tracing::info!(group_id, node_id, "group initialized with single voter");
            }
            Err(e) => {
              let msg = e.to_string();
              if msg.contains("not allowed to initialize") {
                tracing::info!(group_id, node_id, "group reuses existing raft state");
              } else {
                tracing::warn!(
                  group_id,
                  node_id,
  ...
  async fn apply_membership_change(
    ...
    for node_id in to_add {
      ...
      let addr = cluster_meta
        ...
        .ok_or_else(|| ClusterError::InvalidConfig(format!("node {node_id} rpc addr not found")))?;
      ...
      Some(db) => {
        tokio::task::spawn_blocking(move || {
          let iter = match &key_range {
            Some((start, end)) => db.scan(Some(start.as_slice()), Some(end.as_slice()))?,
