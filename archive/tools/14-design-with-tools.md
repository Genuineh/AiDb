...
```json
{
  "path": "/root/code/database/aidb-oldmain/docs/DESIGN_DECISIONS.md"
}
```
...
```json
{
  ...
  "pattern": "设计|决策|RocksDB|OpenRaft"
}
```
...
```json
{
  "pattern": "^\\[features\\]|^default|^cluster|^backup|^monitoring|^compression",
...
{
  "path": "/root/code/database/aidb/docs/modules",
  "pattern": "^## (关键 invariant|已知限制|配置与 feature)",
  "-A": 12
}
```
...
```json
{
  "glob_pattern": "**/wiqun-db-inventory/**",
...
```json
{
  "path": "/root/code/database/wiqun-db/DESIGN.md"
}
```
...
{
  "limit": 80,
  "path": "/root/code/database/aidb-oldmain/docs/THIN_REPLICATION_PLAN.md"
}
```
...
{
  "path": "/root/code/database/AiKv-Workflow/backup/aidb",
  "pattern": "设计|决策|RocksDB|Column|ThinReplication|subcompaction|stall"
}
```
...
```json
{
  ...
  "pattern": "Thin|薄复制|subcompaction|write stall|HTTP"
}
```
...
```json
{
  "glob_pattern": "aidb-oldmain/src/**/*.rs",
  "target_directory": "/root/code/database"
}
```
...
```json
{
  ...
  "pattern": "ThinReplication|monitoring|MetricsServer|ColumnFamily|compact_range"
}
```
...
```json
{
  ...
  "pattern": "pub struct Options|pub ",
  "head_limit": 40
}
```
...
{
  "limit": 50,
  "path": "/root/code/database/aidb-oldmain/src/cluster/thin_replication.rs"
}
```
...
```json
{
  "path": "/root/code/database/aidb-oldmain/src/memtable",
  "pattern": "SkipMap|BTreeMap|RwLock"
}
```
...
```json
{
  ...
  "pattern": "thin_replication|ThinReplication"
}
```
...
{
  "path": "/root/code/database/aidb-oldmain/src",
  "pattern": "subcompaction|write_stall|level0_slowdown|checkpoint|Checkpoint"
}
```
...
{
  "limit": 30,
  "path": "/root/code/database/wiqun-db/src/lib.rs"
}
```
...
```bash
wc -l /root/code/database/aidb-oldmain/src/lib.rs /root/code/database/aidb/src/engine/db/inner.rs 2>/dev/null; find /root/code/database/aidb-oldmain/src -name '*.rs' | wc -l; find /root/code/database/aidb/src -name '*.rs' | wc -l
```
...
```json
{
  "path": "/root/code/database/aidb-oldmain/src/wal",
  "pattern": "WalEntry|text|RecordType"
}
```
...
{
  "limit": 30,
  "path": "/root/code/database/aidb-oldmain/src/cache/lru.rs"
}
```
...
{
  "path": "/root/code/database/AiKv-Workflow/backup/INDEX.md",
  "old_string": "| `DESIGN.md` (总审) | 待开始 | `backup/aidb/DESIGN.md` | 已移出 aidb | modules 完成后定稿 |",
  "new_string": "| `DESIGN.md` (总审) | ✅ | `backup/aidb/DESIGN.md`; `oldmain/DESIGN_DECISIONS.md` (原则); `oldmain/ARCHITECTURE.md` §5; CHANGELOG/superpowers (stall/subcompaction); WiQunTools 09–13 查漏 | 已写 `aidb/DESIGN.md` | 模块级; 无新 ISSUE; ISSUE-001/005/014 根文档一行 |"
}
```
...
{
  "path": "/root/code/database/AiKv-Workflow/backup/PROGRESS.md",
  "old_string": "| 14 | `DESIGN.md` | ⬜ 待开始 | |",
  "new_string": "| 14 | `DESIGN.md` | ✅ 完成 | 2026-06-18 |"
}
```
