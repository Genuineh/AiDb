...
#### 自动回滚机制 (Day 3-4)
- [ ] **ScriptContext设计**
  - [ ] 实现操作缓冲区（基于WriteBatch）
  - [ ] 实现临时读取层（read buffer + DB fallback）
  - [ ] 实现提交和回滚接口
  - [ ] 单元测试
  
- [ ] **LuaExecutor实现**
  - [ ] Lua脚本预处理和编译
  - [ ] 注入db.put/get/delete API
  - [ ] 错误捕获和回滚逻辑
  - [ ] 执行超时控制
  - [ ] 单元测试

#### DB集成 (Day 4-5)
- [ ] 在DB中添加execute_script方法
- [ ] 集成测试：成功场景
- [ ] 集成测试：失败回滚场景
- [ ] 集成测试：并发脚本执行
- [ ] 性能测试
...
**设计要点**：
- ✅ 脚本内所有操作先写入缓冲区，不直接修改DB
- ✅ 脚本执行成功后，使用WriteBatch原子提交
- ✅ 脚本执行失败时，丢弃缓冲区达到自动回滚
- ✅ 支持在脚本内读取同一脚本中的未提交写入
- ✅ 最小化对DB核心代码的修改
