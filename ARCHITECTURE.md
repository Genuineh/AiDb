...
AiDb 是用 Rust 实现的 **嵌入式 LSM-Tree KV 存储库** (lib crate). 单机提供同步 `DB` API; 分布式、备份、监控 (OTel) 通过 Cargo feature 按需启用. **AiDb 不是网络服务** — [AiKv](../aikv/docs/modules/03-storage.md) 在其上实现 Redis RESP 与 Cluster 协议.
...
| 集群 | MetaRaft + MultiRaft, slot 路由, gRPC | `ClusterDataAdapter`, MOVED/ASK, CLUSTER 命令 |
| 备份 | `BackupManager` / `Checkpoint` | `BGSAVE` 直调 `Checkpoint` |
| 指标 | `aidb_*` OTel 系列 (`init`/`init_otel`) | OTLP 导出, `aikv_*` 系列; HTTP 仅 `/health` |
...
│   └── checkpoint/  # 目录一致性快照
├── cluster/         # MetaRaft + MultiRaft (feature cluster)
├── backup/          # BackupManager, RecoveryManager (feature backup)
└── metrics.rs       # OTel 指标 (feature monitoring)
```
...
| [cluster.md](docs/modules/03-cluster.md) | `cluster/*` | MetaRaft, MultiRaft, Router, slot 迁移, gRPC |
...
| `monitoring` | no | `metrics`, `cluster/metrics` | OTel 指标 (global MeterProvider, OTLP 导出) + tracing span |
...
| MetaRaft / MultiRaft | `cluster/meta_raft_node.rs`, `cluster/multi_raft_node.rs` |
...
| 指标注册 (嵌入) | `metrics.rs` — `init` / `init_otel` |
...
- **MultiRaft**: 每 Group 独立 `ShardedStorage` (目录 `data/group_{id}/`) + `OpenRaftNode`.
...
- **OTel Metrics**: `monitoring` feature; `DB::open` 时 `metrics::init()` (需嵌入方已设 global `MeterProvider`).
- **暴露**: AiDb **无内置 HTTP/OTLP 出口**; 嵌入方设置 global `MeterProvider` 并调用 `metrics::init()` 后, `aidb_*` 经嵌入方 OTLP 统一导出 (见 [observability.md](docs/modules/05-observability.md)).
...
4. **指标**: `aikv` 启动时 (otel.rs) 设 global `MeterProvider` 后调用 `aidb::metrics::init()`; `aidb_*` 与 `aikv_*` 共用 OTLP 出口, HTTP 仅 `/health`.
...
- **集群**: OpenRaft + MetaRaft/MultiRaft 分离控制面与数据面; 16384 slot (CRC16, Redis 兼容槽模型).
